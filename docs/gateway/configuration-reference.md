...
```json5
{
  cron: {
    ...
    runLog: {
      ...
      maxAge: "7d", // optional duration string or false
    },
  },
}
```
...
- `runLog.maxAge`: optional duration for pruning visible Run history rows by age, such as `7d` or `24h`; set `false` or omit to disable age pruning.
